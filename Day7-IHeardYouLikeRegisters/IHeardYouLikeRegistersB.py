...
for instruction in instructions:
    ...
    current_highest_register = max(
        registers.items(),
        key=operator.itemgetter(1)
    )[0]

    if registers[current_highest_register] > highest_held_value:
        highest_held_value = registers[current_highest_register]


highest_register_index = max(registers.items(), key=operator.itemgetter(1))[0]

print(registers[highest_register_index])
print(highest_held_value)
